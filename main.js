...
document.getElementById("signOutBtn").addEventListener("click", () => {
  ...
  window.location.href = "profile.html"; // or reload page to trigger redirect
});
  ...
  function addMessageToDOM(msg) {
    ...
    div.innerHTML = `<div id="message-container">
                    <p id="names">${firstname} ${lastname}</p><p id="time">${time}</p>
  ...
  const { data, error } = await supabaseClient.from('messages').insert([{ firstname, lastname, message }]).select();
